            ...
            , {
                ...
                formatter: function (value) {
                    if (value == "-1") {
                        ...
                        return "<span style='color: orange'>正式客户</span>";
                    } else if (value == "2") {
                        return "<span style='color:darkblue'>资源池客户</span>";
                    } else if (value == "-2") {
                        return "<span style='color:greenyellow'>流失客户</span>";
                    }
                }
            }
        ]]
    ...
    var cmdObj = {
        ...
        edit: function () {
            ...
            if (select) {
                if (select.state == false) {
                    ...
                    $.messager.alert("温馨提示", "不能编辑已审核的信息！");
                }
            } else {
            ...
            if (select) {
                if (select.state == "0") {
                    $.messager.confirm("温馨提示", "您确定要这样做吗？", function (bool) {
                        if (bool) {
                            $.post("/potentialCustomer_approve?state=-1", {id: select.id}, function (data) {
                                if (data.success) {
        ...
        submit: function () {
            ...
            if (select) {
                ...
                if (sid) {
                    $.get("/potentialCustomer_" + data[0]['value'] + "?inChargeUser.id=" + sid + "&id=" + select.id, function (data) {
                        if (data.success) {
                            $.messager.alert("温馨提示", data.msg);
